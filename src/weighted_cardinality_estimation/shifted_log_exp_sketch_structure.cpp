...
double ShiftedLogExpSketchStructure::estimate_direct(float logarithm_base) const {
    double tmp_sum = 0.0;
    for (std::uint32_t r: M_) {
        tmp_sum += std::pow(logarithm_base, -(int)(r + offset));
    }
    const double m = (double)M_.size();
    const double k = logarithm_base;
    return (k - 1) * m / (std::log(k) * tmp_sum);
}

double ShiftedLogExpSketchStructure::estimate_newton_cold(float logarithm_base) const {
    const std::uint32_t r_min_val = *std::min_element(M_.begin(), M_.end());
    const double cold_start = std::pow(logarithm_base, (int)(r_min_val + offset));
    return Newton(cold_start, logarithm_base);
}

double ShiftedLogExpSketchStructure::estimate_newton_warm(float logarithm_base) const {
    return Newton(estimate_direct(logarithm_base), logarithm_base);
}
...
size_t ShiftedLogExpSketchStructure::memory_usage_write() const {
    ...
    write_size += sizeof(offset); // 4
    return write_size; // mb/8 + 4
}
