...
size_t FastShiftedLogExpSketch::memory_usage_total() const {
    ...
    total_size += fisher_yates.bytes_total(); // 2m ceil(log_2 m)/8 + 32
    ...
    return total_size; // 2m ceil(log_2 m)/8 + 4m + mb/8 + 65
}

size_t FastShiftedLogExpSketch::memory_usage_write() const {
    size_t write_size = 0;
    write_size += structure.memory_usage_write(); // mb/8 + 4
    write_size += fisher_yates.bytes_write(); // m ceil(log_2 m)/8 + 32
    write_size += sizeof(min_sketch_value); // 4
    write_size += sizeof(min_value_to_change_sketch); // 8
    return write_size; // m ceil(log_2 m)/8 + mb/8 + 48
}
...
double FastShiftedLogExpSketch::estimate_direct() const {
    return structure.estimate_direct(logarithm_base);
}

double FastShiftedLogExpSketch::estimate_newton_cold() const {
    return structure.estimate_newton_cold(logarithm_base);
}

double FastShiftedLogExpSketch::estimate_newton_warm() const {
    return structure.estimate_newton_warm(logarithm_base);
}
