...
template <typename SketchType>
void bind_log_exp_estimators(py::class_<SketchType>& cls) {
    cls.def("estimate_direct", &SketchType::estimate_direct)
       .def("estimate_newton_cold", &SketchType::estimate_newton_cold)
       .def("estimate_newton_warm", &SketchType::estimate_newton_warm);
}
...
PYBIND11_MODULE(_core, m) {
    ...
    {
        auto cls = py::class_<FastLogExpSketch>(m, "FastLogExpSketch")
        ...
        bind_common_sketch_methods(cls);
    }

    {
        auto cls = py::class_<BaseLogExpSketch>(m, "BaseLogExpSketch")
            .def(py::init<std::size_t, const std::vector<std::uint32_t>&, std::uint8_t, float>(),
                py::arg("m"), py::arg("seeds"), py::arg("amount_bits"), py::arg("logarithm_base"))
            .def(py::pickle(
        ...
        bind_common_sketch_methods(cls);
        bind_log_exp_estimators(cls);
        cls.def_readonly("newton_iteration_count", &BaseLogExpSketch::newton_iteration_count);
    }

    {
        auto cls = py::class_<BaseShiftedLogExpSketch>(m, "BaseShiftedLogExpSketch")
            .def(py::init<std::size_t, const std::vector<std::uint32_t>&, std::uint8_t, float>(),
                py::arg("m"), py::arg("seeds"), py::arg("amount_bits"), py::arg("logarithm_base"))
            .def(py::pickle(
    ...
    {
        auto cls = py::class_<FastShiftedLogExpSketch>(m, "FastShiftedLogExpSketch")
            .def(py::init<std::size_t, const std::vector<std::uint32_t>&, std::uint8_t, float>(),
                py::arg("m"), py::arg("seeds"), py::arg("amount_bits"), py::arg("logarithm_base"))
            .def(py::pickle(
    ...
    bind_common_sketch_methods(py::class_<WeightedMinHash>(m, "WeightedMinHash")
        .def(py::init<std::size_t, const std::vector<std::uint32_t>&>(),
            py::arg("m"), py::arg("seeds"))
        .def(py::pickle(
            [](const WeightedMinHash& p) {
                return py::make_tuple(p.get_sketch_size(), p.get_seeds(), p.get_registers());
            },
            [](const py::tuple& t) {
                if (t.size() != 3) throw std::runtime_error("Invalid state for WeightedMinHash pickle!");
