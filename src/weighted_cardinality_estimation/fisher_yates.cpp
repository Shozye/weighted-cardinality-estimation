...
std::uint32_t FisherYates::bytes_total() const {
    size_t total_size = 0;
    total_size += sizeof(rng_engine); // 32
    total_size += permInit.bytes(); // m ceil(log_2 m)/8
    total_size += permWork.bytes(); // m ceil(log_2 m)/8
    return total_size; // 2m ceil(log_2 m)/8 + 32
}
...
std::uint32_t FisherYates::bytes_write() const {
    ...
    total_size += sizeof(rng_engine); // 32
    total_size += permWork.bytes(); // m ceil(log_2 m)/8
    return total_size; // m ceil(log_2 m)/8 + 32
}