...
double BaseLogExpSketch::newton(double c0) const {
    ...
    while (std::abs(c1 - c0) > NEWTON_MAX_ERROR) {
        ...
        if (newton_iteration_count >= NEWTON_MAX_ITERATIONS){ break; }
    }
    return c1;
}

double BaseLogExpSketch::estimate_direct() const {
    return estimate_fast();
}

double BaseLogExpSketch::estimate_newton_cold() const {
    const int r_min_val = *std::min_element(M_.begin(), M_.end());
    return newton(std::pow(logarithm_base, r_min_val));
}

double BaseLogExpSketch::estimate_newton_warm() const {
