...
size_t FastLogExpSketch::memory_usage_write() const {
    ...
    return write_size; // m ceil(log_2 m)/8 + mb/8 + 44
}
...
double FastLogExpSketch::estimate_direct() const {
    double tmp_sum = 0.0;
    for (int r: M_) {
        tmp_sum += std::pow(logarithm_base, -r);
    }
...
double FastLogExpSketch::estimate_newton_cold() const {
    const int r_min_val = *std::min_element(M_.begin(), M_.end());
    return Newton(std::pow(logarithm_base, r_min_val));
}

double FastLogExpSketch::estimate_newton_warm() const {
