...
size_t BaseLogExpSketchJacc::memory_usage_total() const {
    ...
    total_size += sizeof(amount_bits_jaccard); // 1
    total_size += H_.memory_usage_total(); // mb_j/8 + 1
    return total_size; // 4m + mb/8 + mb_j/8 + 23
}

size_t BaseLogExpSketchJacc::memory_usage_write() const {
    size_t total_size = 0;
    total_size += M_.bytes(); // mb/8
    total_size += H_.memory_usage_write(); // mb_j/8
    return total_size; // mb/8 + mb_j/8
}

size_t BaseLogExpSketchJacc::memory_usage_estimate() const {
    size_t total_size = M_.bytes(); // mb/8
    total_size += sizeof(logarithm_base); // 4
    return total_size; // mb/8 + 4
}
